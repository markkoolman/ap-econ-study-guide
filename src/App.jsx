...
const part1=[
  ...
  {id:"ppc",icon:"📊",title:"Production Possibilities Model",sub:"PPC · Opportunity Cost · Efficiency · Growth",tabs:["Simple","Diagram","In-Depth","AP Tips"],cnt:{
    ...
    Diagram:<><PPCDiag/><div className="db" style={{marginTop:".85rem"}}><h5>How to Read the PPC Diagram</h5><p><strong>X-axis:</strong> Quantity of Good B (e.g., consumer goods). <strong>Y-axis:</strong> Quantity of Good A (e.g., capital goods). <strong>The curve itself:</strong> Shows every EFFICIENT combination — using ALL available resources. Points ON the curve = productive efficiency. Points INSIDE the curve = inefficient (wasted resources, like during a recession with unemployment). Points OUTSIDE the curve = currently impossible with existing resources and technology. <strong>The curve bows outward (concave)</strong> because of increasing opportunity cost — as you produce more of one good, you must give up increasing amounts of the other (resources are not perfectly adaptable). <strong>Economic growth</strong> shifts the entire PPC outward — more of BOTH goods become producible. A one-sided improvement (e.g., better farming tech) shifts only the Good B axis intercept outward. Movement along the curve = reallocation of existing resources (opportunity cost).</p></div><Dl t="What Shifts the PPC Outward?" items={["More natural resources (discovery of oil, minerals)","More labor (population growth, immigration)","More capital goods (investment in factories/equipment)","Better technology (innovation, education/training of workers)"]}/></>,
...
const part2=[
  ...
  {id:"gov",icon:"⚖️",title:"Government's Role, Failure & Antitrust",sub:"Ch 5 & 19 — When government doesn't fix markets",tabs:["Simple","In-Depth","Antitrust Deep Dive","AP Tips"],cnt:{
    ...
    "Antitrust Deep Dive":<><B2 c="Antitrust law = the rules that protect competition. Without them, companies would just merge with or destroy all competitors and charge monopoly prices. The US has three main antitrust laws, and AP Micro expects you to know the Sherman Act and the basic types of mergers."/><D t="The Three Antitrust Laws" c="SHERMAN ACT (1890): Section 1 bans collusion — price-fixing cartels, bid-rigging, market allocation among competitors. All per se illegal (automatically illegal, no justification accepted). Section 2 bans monopolization through anticompetitive conduct. Note: legally becoming a monopoly through superior innovation is NOT illegal. Abusing that position to crush competitors IS. CLAYTON ACT (1914): More specific prohibitions — bans price discrimination that harms competition, exclusive dealing contracts, and mergers that substantially lessen competition. FEDERAL TRADE COMMISSION ACT (1914): Created the FTC to investigate and stop unfair competitive practices. FTC and Dept. of Justice both enforce antitrust."/><D t="Three Types of Mergers" c="HORIZONTAL: two direct competitors merge (Exxon + Mobil, two local banks). Directly reduces competition — most scrutinized by regulators. If two firms with large market shares combine, the HHI jumps dramatically. VERTICAL: firm merges with its supplier or distributor (Amazon buying Whole Foods — retailer buying a supplier). Less directly anticompetitive, but can foreclose rivals' access to key inputs or distribution. CONGLOMERATE: firms in completely unrelated industries combine (tobacco company buying a food company). Least antitrust concern — no direct competition between them."/><D t="Herfindahl-Hirschman Index (HHI)" c="The HHI measures how concentrated a market is. Method: for each firm, square its market share (as a percentage). Sum all squared shares. Scale: 0 to 10,000. Examples — Four equal firms (25% each): 25² × 4 = 2,500 (moderately concentrated). Pure monopoly: 100² = 10,000. 100 equal firms (1% each): 1² × 100 = 100 (highly competitive). DOJ merger guidelines: Below 1,500 = competitive (merger usually approved). 1,500–2,500 = moderate concentration (merger scrutinized if it raises HHI by 100+). Above 2,500 = highly concentrated (merger likely challenged if it raises HHI by 200+). AP Micro FRQs occasionally ask you to calculate or interpret HHI."/><AP c={["Sherman Act Section 1 = collusion (illegal per se). Section 2 = monopolization (must show anticompetitive conduct).","Horizontal mergers most AP-tested — directly reduce market competition.","HHI formula: sum of squared market shares. Pure monopoly = 10,000. Calculate quickly on FRQs.","Regulatory capture = the regulated industry ends up controlling the regulator. Classic government failure example."]}/></>,
  ...
  {id:"lorenz",icon:"📊",title:"Income Inequality: Lorenz Curve & Gini Coefficient",sub:"Ch 21 — Measuring and causes of income inequality",tabs:["Simple","Diagram","In-Depth","AP Tips"],cnt:{
    ...
    Diagram:<><LorenzDiag/><div className="db" style={{marginTop:".85rem"}}><h5>How to Read the Lorenz Curve</h5><p><strong>X-axis:</strong> Cumulative percentage of households, ranked from poorest (left) to richest (right). 0% = no households. 100% = all households. <strong>Y-axis:</strong> Cumulative percentage of total income received. <strong>The 45° line (perfect equality):</strong> If income were perfectly equally distributed, the bottom 20% of households would earn exactly 20% of all income, the bottom 50% would earn exactly 50%, etc. Every point would lie exactly on the diagonal. <strong>The actual Lorenz curve:</strong> Bows BELOW the 45° line because income is unequal. Example: in the US, the bottom 20% of households earn about 3-4% of income (far below the 20% they would earn under perfect equality). <strong>Gini coefficient</strong> = Area A (between the 45° line and the Lorenz curve) divided by (Area A + Area B). Ranges from 0 (perfect equality) to 1 (one person owns everything). The more the curve bows outward, the higher the Gini, the more unequal. Government taxes and transfers shift the Lorenz curve TOWARD the diagonal (reduce Gini).</p></div></>,
  ...
  {id:"domtariff",icon:"🌍",title:"Tariff — Domestic Market Analysis",sub:"AP Micro Topic 2.9 — World price, imports, DWL triangles b+d",tabs:["Simple","Diagram","In-Depth","AP Tips"],cnt:{
    ...
    Diagram:<><DomTariffDiag/><div className="db" style={{marginTop:".85rem"}}><h5>How to Read the Domestic Tariff Diagram</h5><p><strong>X-axis:</strong> Quantity of the good (e.g., steel). <strong>Y-axis:</strong> Price per unit ($). <strong>Before the tariff:</strong> World price (Pw) is below domestic equilibrium. Country imports the difference between Qd (domestic demand at Pw) and Qs (domestic supply at Pw). Imports = Qd − Qs. <strong>After the tariff:</strong> Domestic price rises to Pw + tariff. Domestic supply increases from Qs to Qs2 (domestic producers can now profitably supply more at the higher price). Domestic demand falls from Qd to Qd2. Imports shrink from (Qd − Qs) to (Qd2 − Qs2). <strong>The four areas (a, b, c, d):</strong> Area a = gained by domestic producers (transferred from consumers). Area b = DWL (overproduction — domestic producers making units that cost more than world price). Area c = government tariff revenue. Area d = DWL (underconsumption — consumers priced out). Net welfare effect: consumers lose (a + b + c + d). Producers gain (a). Government gains (c). Net loss = b + d (the two DWL triangles).</p></div></>,
...
const part4=[
  ...
  {id:"costs",icon:"💰",title:"Businesses & Costs of Production",sub:"Ch 9 — Explicit, implicit, economic profit, cost curves",tabs:["Simple","Diagram","In-Depth","AP Tips"],cnt:{
    ...
    Diagram:<><CostDiag/><div className="db" style={{marginTop:".85rem"}}><h5>How to Read the Cost Curves Diagram</h5><p><strong>X-axis:</strong> Output (Q). <strong>Y-axis:</strong> Cost per unit ($). <strong>MC (Marginal Cost):</strong> U-shaped — initially falls (specialization, efficiency gains as output rises) then rises (diminishing returns). This is the MOST IMPORTANT curve. <strong>ATC (Average Total Cost):</strong> U-shaped, wider curve above AVC. ATC = AFC + AVC. The minimum of ATC is where MC = ATC (MC crosses ATC at its lowest point — ALWAYS). <strong>AVC (Average Variable Cost):</strong> U-shaped, inside ATC. Minimum of AVC is also where MC = AVC. <strong>AFC (Average Fixed Cost):</strong> Continuously falling hyperbola — fixed costs spread over more units. <strong>Key rule:</strong> When MC is below ATC, it pulls ATC DOWN. When MC is above ATC, it pulls ATC UP. That is why MC always passes through the minimum of both ATC and AVC. This is a mathematical necessity, not a coincidence.</p></div></>,
  ...
  {id:"pc",icon:"🏭",title:"Pure Competition — Short & Long Run",sub:"Ch 10–11 — Price takers, MR=MC, LR normal profit",tabs:["Simple","Diagram","In-Depth","AP Tips"],cnt:{
    ...
    Diagram:<><PCFirmDiag/><LRIndustryDiag/><div className="db" style={{marginTop:".85rem"}}><h5>How to Read the Side-by-Side Diagrams</h5><p><strong>LEFT (Market):</strong> Normal supply and demand. Many firms together determine market price P*. X-axis = total market quantity Q. <strong>RIGHT (Individual Firm):</strong> One tiny firm taking P* as given. The firm sees a HORIZONTAL demand line at P* — it can sell any amount at P* but cannot charge more (would lose all customers) and has no reason to charge less. This horizontal line is SIMULTANEOUSLY the firm's demand curve, its MR (marginal revenue) curve, and its AR (average revenue) curve — all equal to P*. <strong>Finding profit-max output:</strong> Go right until MC = MR (= P*). Drop down to Q-axis to find the firm's quantity. <strong>Profit/loss rectangle:</strong> Height = P* − ATC at that Q. Width = firm's Q. Positive rectangle = profit. Negative = loss. <strong>Long-run equilibrium:</strong> P* = minimum ATC. Rectangle disappears (zero economic profit). MC = MR = P* = min ATC. Productive AND allocative efficiency.</p></div></>,
  ...
  {id:"mono",icon:"🏰",title:"Pure Monopoly",sub:"Ch 12 — Price maker, MR < P, regulation",tabs:["Simple","Diagram","In-Depth","Price Discrimination","AP Tips"],cnt:{
    ...
    Diagram:<><MonopDiag/><div className="db" style={{marginTop:".85rem"}}><h5>How to Read the Monopoly Diagram — Step by Step</h5><p><strong>X-axis:</strong> Quantity (Q). <strong>Y-axis:</strong> Price/Cost ($). <strong>Demand (D = AR):</strong> Downward sloping — monopolist faces the whole market demand. Also the Average Revenue curve. <strong>MR (Marginal Revenue):</strong> Below D and steeper — because to sell one more unit, the monopolist must lower the price for ALL units, so MR falls faster than price. MR hits zero where demand is unit elastic. <strong>Step 1: Find Qm</strong> — where MR = MC (the profit-max rule, same as always). Drop straight down to the x-axis. <strong>Step 2: Find Pm</strong> — rise vertically from Qm up to the DEMAND curve (NOT the MR curve). This is the monopoly price. <strong>Step 3: Find ATC at Qm</strong> — read ATC off the ATC curve at Qm. <strong>Profit rectangle:</strong> (Pm − ATC) × Qm. Shaded above ATC up to Pm. <strong>DWL triangle:</strong> Between Qm and the competitive quantity (where MC meets Demand), bounded by demand above and MC below.</p></div></>,
    ...
    "Price Discrimination":<><B2 c="Price discrimination means charging DIFFERENT PRICES to different customers for the SAME product — not because costs differ, but because the firm can identify who will pay more. Airlines do this: business traveler books last-minute, pays $800. Leisure traveler booked 3 months ahead, pays $180. Same plane, same seat. The airline is capturing consumer surplus — extracting maximum willingness to pay from each buyer. This increases profit and sometimes reduces DWL."/><D t="Three Degrees — Know These for AP" c="FIRST DEGREE (perfect): charge each buyer their exact maximum willingness-to-pay. Captures ALL consumer surplus. Zero DWL (every willing buyer gets the good). Impossible in practice — you would need to read minds. SECOND DEGREE: quantity-based pricing (bulk discounts). Buy 1-10 units at $5, buy 11-50 at $4. Price falls as quantity rises. Common in wholesale markets. THIRD DEGREE (most common, most AP-tested): charge different GROUPS different prices based on their demand elasticity. Examples: student discounts at movies and software (students have elastic demand — they will not pay full price), airline business vs. economy class, geographic pricing (same drug costs more in the US than in Canada). Required conditions: (1) firm has market power, (2) can identify and separate groups by elasticity, (3) can prevent resale between groups."/><D t="Effect on Output, Profit, and Welfare" c="Without discrimination: monopolist picks one price at MR=MC, serves only customers willing to pay that price. Leaves money uncaptured above that price, and excludes customers who would pay more than MC but less than the monopoly price. With third-degree discrimination: charge high-elasticity group less (serves more customers), charge low-elasticity group more (captures more surplus). Total output RISES compared to single-price monopoly. Profit rises. Consumer surplus falls or disappears — transfers to producer. Society may benefit from more output, but consumers are worse off. AP Micro FRQ: if asked to compare discriminating vs. non-discriminating monopolist — discriminating firm produces MORE and earns MORE profit."/><AP c={["Price discrimination requires market power + separable groups with different elasticities + no resale.","Third degree most tested: name a real example (airlines, student discounts, software tiers).","Perfect price discrimination = zero consumer surplus, zero DWL, output equals competitive level.","AP FRQ phrasing: If asked whether discrimination increases output — YES compared to single-price monopoly."]}/></>,
...
const part5=[
  {id:"rm",icon:"👷",title:"Factor Markets: Wages, Rent, Interest & Profit",sub:"Ch 14–16 — Derived demand, MRP, monopsony, loanable funds",tabs:["Simple","Diagrams","In-Depth","AP Tips"],cnt:{
    ...
    Diagrams:<><LaborDiag/><MonopsonyDiag/><LFDiag/><div className="db" style={{marginTop:".85rem"}}><h5>How to Read These Labor Market Diagrams</h5><p><strong>COMPETITIVE LABOR (left):</strong> X-axis = quantity of labor; Y-axis = wage rate. Supply of labor (SL) slopes UP — higher wages attract more workers into this market. Demand for labor (DL = MRP) slopes DOWN — each additional worker adds less revenue (diminishing returns). Equilibrium W* and L* where SL = DL. Every worker is paid exactly their MRP. <strong>MONOPSONY (right):</strong> One employer faces the whole upward-sloping SL curve. To hire one MORE worker, must raise wage for ALL current workers — so Marginal Resource Cost (MRC) rises FASTER than the supply curve — MRC is above SL and steeper. Monopsonist hires where MRP = MRC (the profit-max rule for inputs). Find quantity Lm on x-axis, then drop to SUPPLY CURVE (not MRC) to read wage Wm. Result: Lm less than competitive L*, Wm less than competitive W*. Workers paid below their MRP. DWL triangle between Lm and L*, bounded by MRP above and SL below.</p></div></>,
...
const part6=[
  ...
  {id:"growth",icon:"🚀",title:"Economic Growth",sub:"Ch 26 — Determinants, productivity, Rule of 70",tabs:["Simple","In-Depth","Technology & Innovation","AP Tips"],cnt:{
    ...
    "Technology & Innovation":<><B2 c="Why did living standards rise 20-fold in the US between 1870 and today? TECHNOLOGY. Better methods of production let us produce more with the same inputs. This is the single most important driver of long-run economic growth and directly shifts the LRAS rightward — more output at every price level."/><D t="Sources of Technological Progress" c="(1) Research and Development: firms fund R&D when they can profit from innovations (patents, first-mover advantage). Government funds basic research with no immediate commercial application (DARPA created the internet; NIH funded mRNA vaccine research). (2) Patents: give innovators temporary monopoly power → creates incentive to invest → knowledge eventually becomes public. Tradeoff: short-run monopoly inefficiency vs. long-run innovation benefit. (3) Human capital: educated, well-trained workers generate more innovations and adopt new technologies faster. (4) Learning by doing: firms get better at production through experience — productivity rises automatically over time."/><D t="Creative Destruction" c="Joseph Schumpeter's concept: capitalism advances through creative destruction — new technologies destroy old industries. The car destroyed the horse-and-buggy industry. Netflix destroyed Blockbuster. Smartphones destroyed camera and GPS device makers. This process is economically efficient (resources shift to more productive uses) but socially disruptive (workers in old industries lose jobs and must retrain). AP Macro links this to long-run LRAS shifts and supply-side policy debates."/><D t="Innovation vs. Imitation" c="Not every country needs to invent new technologies — developing countries can adopt technologies that advanced countries already created. This technology transfer is why South Korea and China grew so rapidly — they adopted proven production methods rather than starting from zero. Once a country approaches the global technology frontier (the US, Germany, Japan), it must rely on its own innovation rather than imitation to continue growing — which is why advanced countries invest heavily in universities and R&D."/><AP c={["Technology = biggest long-run growth driver. Shifts LRAS rightward. Raises living standards permanently.","Patents: temporary monopoly to incentivize innovation. AP Micro FRQ may test this tradeoff.","Creative destruction: new technologies destroy old industries. Feature, not bug, of market economies.","Supply-side growth policies: R&D tax credits, education investment, deregulation — all shift LRAS right."]}/>   </>,
...
const part7=[
  ...
  {id:"reservemkt",icon:"🏦",title:"The Reserve Market",sub:"AP Macro Topic 4.6 — Ample reserves, IOR, policy rate",tabs:["Simple","Diagram","In-Depth","AP Tips"],cnt:{
    ...
    Diagram:<><ReserveMktDiag/><div className="db" style={{marginTop:".85rem"}}><h5>How to Read the Reserve Market Diagram (Ample Reserves)</h5><p><strong>X-axis:</strong> Quantity of bank reserves ($ billions). <strong>Y-axis:</strong> Interest rate on reserves / federal funds rate (%). <strong>Demand for reserves (DR):</strong> Downward sloping — at higher interest rates, banks want to hold fewer reserves (opportunity cost of holding reserves is high). <strong>Supply of reserves (SR):</strong> In the AMPLE RESERVES framework (post-2008), this is HORIZONTAL at the IOR (Interest on Reserves) rate. Why horizontal? Because the Fed sets IOR as a floor — no bank will lend to another bank at a rate BELOW what the Fed pays for just parking money safely. So all market rates stay at or above IOR. <strong>Equilibrium:</strong> Where DR intersects the horizontal SR = the policy rate. <strong>Fed raises IOR:</strong> SR shifts UP → equilibrium rate rises → all interest rates in the economy rise → contractionary effect. <strong>Fed lowers IOR:</strong> SR shifts DOWN → rates fall → expansionary effect. This is how the Fed actually operates today — not by shifting money supply volume, but by adjusting the IOR floor.</p></div></>,
  ...
  {id:"adasm",icon:"📐",title:"Aggregate Demand & Aggregate Supply",sub:"Ch 30 — The core macro model",tabs:["Simple","Diagram","In-Depth","AP Tips"],cnt:{
    ...
    Diagram:<><ADASDiag/><div className="db" style={{marginTop:".85rem"}}><h5>How to Read the AD-AS Diagram</h5><p><strong>X-axis:</strong> Real GDP (total inflation-adjusted output of the entire economy). <strong>Y-axis:</strong> Price Level (an index of all prices — like the GDP deflator). <strong>AD curve:</strong> Downward sloping for three reasons: (1) Wealth effect — higher price level erodes real wealth → less spending. (2) Interest rate effect — higher prices → need more money to transact → interest rates rise → investment falls. (3) Foreign purchases effect — higher US prices → exports fall, imports rise → Xn falls. <strong>SRAS:</strong> Upward sloping — wages and input prices are sticky in the short run, so as price level rises, firms earn more profit and produce more. <strong>LRAS:</strong> Vertical at potential GDP (Yp) — in the long run, output is determined by resources and technology only, not price level. <strong>Recessionary gap:</strong> Equilibrium real GDP is to the LEFT of LRAS — economy producing below capacity, unemployment above NRU. <strong>Inflationary gap:</strong> Equilibrium is to the RIGHT of LRAS — economy overheating, unemployment below NRU, wages rising.</p></div><Dl t="AD Shifters (CIGXN)" items={["C: Consumer wealth, confidence, taxes, debt","I: Interest rates, business confidence, technology, investment tax credits","G: Government spending changes","Xn: Exchange rates, foreign income changes, tariffs"]}/><Dl t="SRAS Shifters" items={["Input prices (oil, wages, raw materials)","Productivity changes","Business taxes / subsidies","Government regulations"]}/></>,
  ...
  {id:"mp",icon:"🏦",title:"Money, Banking & Monetary Policy",sub:"Ch 32–34 — Fed, money supply, tools, money market",tabs:["Simple","Diagrams","In-Depth","AP Tips"],cnt:{
    ...
    Diagrams:<><MoneyMktDiag/><MoneyMultDiag/><div className="db" style={{marginTop:".85rem"}}><h5>How to Read the Money Market and Loanable Funds Diagrams</h5><p><strong>MONEY MARKET (traditional framework):</strong> X-axis = quantity of money; Y-axis = nominal interest rate (i). Money Supply (SM) = VERTICAL — the Fed directly controls the money supply regardless of interest rates. Money Demand (DM) slopes down (at higher rates, people prefer interest-bearing assets over idle cash). Fed buys bonds → injects money → SM shifts RIGHT → i falls → investment rises. <strong>LOANABLE FUNDS MARKET:</strong> X-axis = quantity of loanable funds; Y-axis = real interest rate (r). Supply = household saving (upward sloping). Demand = investment borrowing (downward sloping). Government deficit spending shifts demand RIGHT → r rises → crowding out private investment. <strong>Key distinction for AP:</strong> Money market uses NOMINAL rate and VERTICAL supply. Loanable funds uses REAL rate and UPWARD supply. Both appear on AP Macro FRQs — read the question carefully to know which diagram to draw.</p></div></>,
  ...
  {id:"tacct",icon:"📋",title:"T-Accounts & Bank Balance Sheets",sub:"AP Macro Topic 4.4 — How banks create money, OMO mechanics",tabs:["Simple","Diagram","In-Depth","Money Creation Step-by-Step","AP Tips"],cnt:{
    ...
    "Money Creation Step-by-Step":<><B2 c="One of the most tested topics on AP Macro: how does one bank deposit turn into MANY times that amount in new money across the whole banking system? This is the MONEY MULTIPLIER in action. Each bank creates money by keeping a fraction as reserves and lending the rest, and that lending process repeats through the entire system."/><D t="Full Trace: $1,000 Deposit, RR = 20%" c="Round 1 — Bank A receives $1,000 deposit. Required reserves = $200. Loans out $800. New money created: $800. Round 2 — Borrower deposits $800 in Bank B. Required reserves = $160. Loans out $640. New money created: $640. Round 3 — That $640 deposited in Bank C. Required reserves = $128. Loans out $512. New money created: $512. Pattern: each round = previous round × 0.80 (MPC of deposits = 1 − RR). This is a geometric series. Total new deposits = $1,000 × (1/0.20) = $5,000. New money BEYOND the original deposit = $4,000. The original $1,000 + $4,000 new = $5,000 total."/><D t="OMO from the Public vs. from a Bank" c="Fed buys $1,000 bond FROM THE PUBLIC: Person gets $1,000 check → deposits it in their bank → bank has new $1,000 deposit → keeps 20% ($200), loans $800 → multiplier runs → total new money = $5,000. Fed buys $1,000 bond FROM A BANK: Bank receives $1,000 in reserves (no new deposit). Bank has $1,000 in new excess reserves → loans $1,000 out → borrower deposits it → multiplier runs → total new money = $1,000 × (1/0.20) = $5,000. Key difference: when buying from a bank, the FULL $1,000 is available to loan (not just 80% like in the public scenario), so the first round of new lending is larger."/><D t="What Limits the Actual Multiplier?" c="The theoretical money multiplier (1/RR) assumes ALL excess reserves are loaned AND all loans are re-deposited. In practice, two leakages reduce it: (1) Banks hold excess reserves — especially in recessions or when IOR (interest on reserves) is high enough that banks prefer parking money at the Fed to making risky loans. After 2008, US banks held over $2 trillion in excess reserves — the actual multiplier collapsed far below 1/RR. (2) Cash holdings — if borrowers keep cash instead of re-depositing, the multiplier shrinks. The real-world multiplier is always less than 1/RR."/><AP c={["Trace all three rounds of the multiplier on FRQs. Show each bank's T-account — graders give points per step.","OMO from bank: reserves up, securities down, deposits unchanged initially. Then bank loans excess reserves.","OMO from public: deposits up, reserves up. Bank keeps RR fraction, loans rest. Full multiplier applies.","If RR = 10%: multiplier = 10. $1,000 new reserves → up to $10,000 new money. Show calculation."]}/></>,
  ...
  {id:"pc2",icon:"📈",title:"Phillips Curve, Extended AS & Supply-Side",sub:"Ch 36–37 — SR/LR Phillips, stagflation, Laffer curve",tabs:["Simple","Diagram","In-Depth","Competing Macro Theories","AP Tips"],cnt:{
    ...
    Diagram:<><PhillipsDiag/><div className="db" style={{marginTop:".85rem"}}><h5>How to Read the Phillips Curve Diagram</h5><p><strong>X-axis:</strong> Unemployment rate (%). <strong>Y-axis:</strong> Inflation rate (%). <strong>SRPC (Short-Run Phillips Curve):</strong> Downward sloping — as unemployment falls (economy expands), inflation rises. Movement along SRPC happens when AD changes: AD right = move left (lower U, higher inflation). AD left = move right (higher U, lower inflation). <strong>LRPC (Long-Run Phillips Curve):</strong> Vertical at the Natural Rate of Unemployment (NRU) — about 4-5% in the US. In the long run there is NO tradeoff: you cannot permanently lower unemployment below NRU by accepting higher inflation. Workers adjust expectations. <strong>SRPC shifts:</strong> A supply shock (oil price spike) shifts the entire SRPC RIGHTWARD — now you get more inflation at every unemployment rate (stagflation). This is the worst case for policymakers.</p></div></>,
    ...
    "Competing Macro Theories":<><B2 c="Economists disagree on how the macroeconomy works and what government should do. Three major schools give very different policy prescriptions for the same problem. AP Macro expects you to distinguish these views, especially on FRQs asking how 'a classical economist' or 'a Keynesian' would respond."/><D t="Keynesian Economics" c="The economy can get stuck in recession and NOT self-correct quickly. Wages and prices are STICKY downward — workers resist pay cuts, firms resist price cuts — so the economy stays below potential for years. Solution: active fiscal stimulus (G up, T down) to shift AD right and close the gap quickly. Keynesians: discretionary policy is necessary and effective. The multiplier amplifies fiscal stimulus. 'In the long run, we are all dead' — waiting for self-correction is not acceptable."/><D t="Monetarism (Friedman)" c="Money is neutral in the long run: increasing M only raises P, not real Q. The Fed's biggest mistakes (especially letting money supply collapse in 1930-33, turning a recession into the Great Depression) prove that discretionary policy does more harm than good. Solution: a k-PERCENT RULE — the Fed should grow money supply at a constant rate matching long-run real GDP growth (~3-4%/year). This prevents both inflation AND policy-induced recessions. No discretion. No fine-tuning."/><D t="Supply-Side / New Classical" c="Markets clear quickly — wages and prices are flexible — so the economy self-corrects rapidly without government help. Instead of stimulating AD (which just causes inflation at full employment), focus on shifting LRAS rightward: cut marginal tax rates to increase incentives to work and invest, deregulate to reduce business costs, invest in education and infrastructure to boost productivity. Laffer Curve: very high tax rates reduce economic activity so much that cutting rates might actually INCREASE government revenue."/><D t="Policy Comparison Summary" c="Recessionary gap — what should policy do? Keynesian: stimulate AD (fiscal stimulus). Monetarist: do nothing (or maintain steady money growth rule). Supply-side: cut taxes and deregulate to shift LRAS. All three agree: in the long run, economy returns to Yp. They disagree on HOW FAST and WHAT POLICY to use."/><AP c={["Three schools: Keynesian (AD stimulus), Monetarist (k-percent rule), Supply-side (LRAS shifts). Know all three.","Laffer Curve: revenue = 0 at 0% rate AND 100% rate. Peak somewhere in between. Conceptual AP question.","Keynesian vs. supply-side: Keynesian shifts AD. Supply-side shifts LRAS. Different curves, different effects on price level.","AP FRQ: if asked how a classical economist responds to recession — they say wait for self-correction via falling wages."]}/></>,
